...
unsigned char cache_read_byte(membase_t *mb, addr_t address) {
    ...
    p_cache->num_reads++;
    p_line->last_time_accessed = clock_tick();
...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    assert(set != NULL);
    assert(offset != NULL);
    uint32_t offset_mask = (p_cache->block_size - 1);
    uint32_t seti_mask = (p_cache->num_sets  - 1) << p_cache->block_offset_bits;
    uint32_t tag_mask = ~(seti_mask | offset_mask);
    *offset = offset_mask & address;
    *set = (seti_mask & address) >> p_cache->block_offset_bits;
    *tag = (tag_mask & address) >> (p_cache->block_offset_bits + p_cache->sets_addr_bits); 
}
 ...
 */
addr_t get_block_start_from_address(cache_t *p_cache, addr_t address) {
    return (~(p_cache->block_size - 1)) & address;
}
...
addr_t get_block_start_from_line_info(cache_t *p_cache,
                                      addr_t tag, addr_t set_no) {
    addr_t address = ((tag << p_cache->sets_addr_bits) | set_no) << p_cache->block_offset_bits;
    return address;
}
...
cacheline_t * find_line_in_set(cacheset_t *p_set, addr_t tag) {
   ...
   cacheline_t *found_line = p_set->cache_lines;

    for (i = 0; i < p_set->num_lines; i++)
    {
        if (found_line[i].valid == 1 && found_line[i].tag == tag)
        {
            return &(found_line[i]);
        }
    }
 ...
 */
cacheline_t * choose_victim(cacheset_t *p_set) {
    cacheline_t *victim = p_set->cache_lines;
    int i;
    int i_victim = 10000000;
    unsigned long long int smallest_time;
    smallest_time = clock_tick();
    ...
    for (i = 0; i < p_set->num_lines; i++)
    {
        ...
        if (victim[i].last_time_accessed < smallest_time)
        {
            i_victim = i;
            smallest_time = victim[i].last_time_accessed;
        }
    }
    return &(victim[i_victim]);
#endif
