...
void Shape_setDensity(Shape_Data *this, float D) {
    assert(D > 0); // The above comment says nonnegative, but the sample code
    // from shapes.h displays a positive assert, so I'm going with that.
    this->density = D;
...
void Box_init(Box_Data *this, Box_Class *class,
    ...
    Shape_init((Shape_Data *) this, &Shape, D);
    this->class = class;
...
Box_Data * new_Box(float L, float W, float H, float D) {
    Box_Data *new_box = malloc(sizeof(Box_Data));
...
Sphere_Data * new_Sphere(float R, float D) {
    Sphere_Data *new_sphere = malloc(sizeof(Sphere_Data));
...
float Sphere_getVolume(Sphere_Data *this) {
    return 1.3333 * 3.1415926 * this->radius * this->radius * this->radius;
}
...
Cone_Data * new_Cone(float BR, float H, float D) {
    Cone_Data *new_cone = malloc(sizeof(Cone_Data));
...
float Cone_getVolume(Cone_Data *this) {
    return 1.04719755 * this->base_radius * this->base_radius * this->height;
}
